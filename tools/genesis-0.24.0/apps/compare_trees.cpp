...
int main( int argc, const char* argv[] )
{
    ...
    Tree inferred_tree = CommonTreeNewickReader().read(from_file(argv[2]));
    
    auto printer = PrinterCompact();
    
    std::cout << printer.print(orig_tree) << std::endl;
    std::cout << printer.print(inferred_tree) << std::endl;

    std::cout << "RF distance: " << rf_distance_relative(orig_tree, inferred_tree) << "\n";
