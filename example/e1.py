...
def run_raxml_rfdist_2trees(folder_path, tree1, tree2):
    """
    Uses RAxML-NG to compute the RF-distance
    @param folder_path: data set directory
    @param tree1: path to first tree
    @param tree2: path to second tree
    @return:
    """
    tree1_path = tree1
    tree2_path = tree2
    command = f"{RAXML_NG_PATH} --rfdist {tree1_path},{tree2_path} --redo"

    ret = -1
    try:
        out = subprocess.run(command.split(), cwd=folder_path, check=True, stdout=subprocess.PIPE).stdout.decode("utf-8")
...
def toy_study(num_points, num_repeats=NUM_REPEATS):
    ...
    for folder in folders:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            # Compute the average RF-distance between re-inferred trees and the true tree
            rf_dists = [run_raxml_rfdist_2trees(folder_path, "tree_best.newick", f"{i}.raxml.bestTree")
                        for i in range(0, num_repeats)]
            rf = statistics.mean(rf_dists)

            """
            # One alternative could be to compare the RF-distances of the re-inferred trees among each other, 
            # without comparing them to the true tree
